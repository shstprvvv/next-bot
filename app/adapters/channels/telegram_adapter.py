...
class TelegramAdapter:
    ...
    async def handle_incoming_message(self, event):
        ...
        # Если фото пришло без текста, НЕ ставим раннюю заглушку, 
        # а скачиваем и прокидываем дальше в UseCase.
        if is_audio:
    ...
    async def process_messages(self, user_id, event):
        ...
        # Если прислали только фото без текста, не ругаемся, а идем дальше!
        if not full_message and image_base64:
             logger.info(f"[Telegram] Получено только фото без текста от {user_id}. Идем на распознавание.")

...
class AnswerQuestionUseCase:
    ...
    async def execute(self, user_id: int | str, question: str, history: list[str] = None, source: str = "telegram", image_base64: str = None) -> str:
        ...
        if image_base64:
            ...
            vision_prompt = [
                {"type": "text", "text": "Ты эксперт технической поддержки. Клиент прислал фото своей проблемы с ТВ-приставкой. Твоя задача — внимательно изучить фото и детально описать: 1. Что изображено на экране (текст ошибок, интерфейс, меню, логотипы)? 2. Как подключены провода к приставке или телевизору (если они видны)? 3. Горят ли какие-нибудь индикаторы на устройстве? Если текста или явной ошибки нет, опиши общую ситуацию."},
...
- Удали из вопроса лишние эмоции, маты, угрозы возвратом и "воду". Оставь только техническую суть проблемы.
- Раскрой все местоимения (он, она, это), опираясь на историю.
- Если клиент просто поздоровался, прислал смайлик или в его сообщении нет технического вопроса (например, "жду", "в смысле?"), напиши строго фразу: 'нет конкретной проблемы'.
